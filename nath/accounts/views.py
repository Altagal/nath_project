...
from nath.settings import DEFAULT_PASSWORD
...
def account_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Registrado com sucesso. Entre em contato com o administrador da aplicação para mais detales.",
            )
...
def account_read(request, pk):
    context = {"account": Account.objects.get(id=pk)}
...
def account_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if (
            not obj.is_active
        ):  # Se usuario foi desativado, retira ele de todos os grupos
...
def account_reset_password(request, pk):
    ...
    messages.success(
        request,
        "Senha redefinida com sucesso. Ultilize "
        " + DEFAULT_PASSWORD + "
        " no proximo acesso.",
    )
